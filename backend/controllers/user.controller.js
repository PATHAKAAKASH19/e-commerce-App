...
async function login(req) {
    ...
    try {
       ...
       const isRefreshTokenExist = await RefreshToken.findOne({user: userExist._id})
...
async function generateAccessToken(req, res) {
    ...
    if(!refreshToken) return res.status(403).json({message: "refresh token not found please login"})

    try {
       
        const refreshToken = await RefreshToken.findOne({refreshToken : refreshToken}) 
        if(!refreshToken) return res.status(403).json({message : "invalid token"})
...
async function deleteAccount(req, res){
    ...
    try {
        ...
        const deleteRefreshTokenFromDatabase = await RefreshToken.findOneAndDelete({refreshToken})
